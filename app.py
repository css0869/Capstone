...
def get_ig_list(df,name):
    ...
    l=re.split('[,]',df['Ingredient'][idx].split(".")[0])
    l=[x.strip().lower().rstrip(".") for x in l  ]
    l=[x for x in l if not x.startswith(('*',"please","water","aqua","eau","ingredient")) and not x.isdigit()]
    return l
...
def app():       
    ...
    #option = st.sidebar.selectbox('I like this product',df['Brand']+[": "]+df['Name'])
    ...
    allergen=st.sidebar.selectbox("Which ingredient are you sensitive/allergic to?", sorted(get_ig_list(df,prod)))
